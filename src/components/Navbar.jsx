        ...
        <>
            <nav className="fixed top-0 left-0 right-0 z-10 rounded-bl-lg rounded-br-lg text-background ">
                    ...
                    <div className="menu hidden md:block md:w-auto" id="navbar">
                        <ul className="flex flex-row mr-5 justify-between gap-2 ">
                            {navLinks.map((link, index) => (
                                <li className='text-text hover:text-primary font-Lora lg:text-2xl md:text-lg ' key={index}>
                ...
                <div className='transition ease-in-out'>
                    {navbarOpen ? <div className='absolute right-0 flex justify-end before:bg-gradient-to-r before:from-transparent before:to-secondary before:w-[35dvw] before:opacity-50 '>
            <ul className="text-text h-[calc(100vh-3.5rem)] w-80 flex flex-col gap-5 justify-center items-center text-3xl z-10 bg-background md:hidden ">
                {navLinks.map((link, index) => (
                    <li className='hover:text-primary' key={index} onClick={() => setNavbarOpen(false)}>
                        <NavLink href={link.path} title={link.title} />
                    </li>
                ))}
            </ul>
